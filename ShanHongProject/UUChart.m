...
@property (strong, nonatomic) UULineChart * lineChart; //折线图

@property (strong, nonatomic) UUBarChart * barChart; //柱状图

@property (assign, nonatomic) id<UUChartDataSource> dataSource; //设置delegate对象
...
-(void)setUpChart{
	if (self.chartStyle == UUChartLineStyle) {
        ...
        if([self.dataSource respondsToSelector:@selector(UUChart:ShowVericationLineAtIndex:)])
        {
            ...
            [_lineChart setShowVericationLine:showVericationArray];
        }
